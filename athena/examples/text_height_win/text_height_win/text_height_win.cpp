// text_height_win.cpp : Defines the entry point for the console application.
//
...
void write_text_height(const image_t&     image,
		      const std::size_t& text_height) {
  std::string hwt_fname = "text_height.txt";
  std::fstream hwt_fs(hwt_fname.c_str(),std::fstream::out); 
  //hwt_fs << "hwt = [" << image.size().height << "," << image.size().width << "," << text_height << "];";
  hwt_fs <<text_height;
  hwt_fs.close();
...
int main(int argc,char** argv) {  
  ...
  write_text_height(image,text_height);
